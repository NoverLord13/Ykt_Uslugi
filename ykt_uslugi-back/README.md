...
Команды выполняются из `ykt_uslugi-back`, иначе относительный путь `uploads` укажет в другой каталог. PostgreSQL должен быть доступен по `DATABASE_URL`.
...
├── database.py    # async engine, async session, seed категорий
├── dependencies.py
└── main.py
```
