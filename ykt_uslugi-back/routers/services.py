...
async def _validate_category_pair(db: AsyncSession, category_id: int | None, subcategory_id: int | None) -> None:
...
async def list_services(
    ...
    stmt = (
        ...
        .where(Service.is_active.is_(True), Service.status == "active")
    )
...
async def list_my_services(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_service(
    ...
    if current_user and service.owner_id == current_user.id:
        has_access = True
    if current_user and await db.scalar(
...
async def create_service(
    ...
    await _validate_category_pair(db, category_id, subcategory_id)
    title = title.strip()
...
async def update_service(
    ...
    if category_id is not None or subcategory_id is not None or clear_category or clear_subcategory:
        ...
        await _validate_category_pair(db, next_category_id, next_subcategory_id)
...
async def get_similar_services(
    ...
    if len(results) < limit and service.category_id is not None:
        ...
        result = await db.execute(
            base_stmt
            .where(Service.category_id == service.category_id, Service.id.notin_(existing_ids or {-1}))
