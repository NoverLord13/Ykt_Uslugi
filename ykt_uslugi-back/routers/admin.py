...
async def list_users(skip: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=100), _: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(User).order_by(User.created_at.desc()).offset(skip).limit(limit))
    users = result.scalars().all()
    return ApiResponse(message="Список пользователей", data=[UserRead.model_validate(u) for u in users])


@router.patch("/users/{user_id}", response_model=ApiResponse[UserRead])
async def update_user(user_id: int, body: AdminUserUpdate, admin: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
...
async def list_all_services(skip: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=100), _: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Service).options(*_service_summary_options()).order_by(Service.created_at.desc()).offset(skip).limit(limit))
    services = result.unique().scalars().all()
    return ApiResponse(message="Список всех услуг", data=[ServiceSummaryRead.model_validate(s) for s in services])


@router.patch("/services/{service_id}", response_model=ApiResponse[ServiceRead])
async def moderate_service(service_id: int, body: AdminServiceUpdate, _: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
...
async def admin_delete_service(service_id: int, _: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    service = await db.scalar(select(Service).where(Service.id == service_id).with_for_update())
    if not service:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Услуга не найдена")
...
async def admin_list_categories(_: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Category).options(selectinload(Category.subcategories)).order_by(Category.name.asc()))
...
async def admin_create_category(body: CategoryCreate, _: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    ...
    db.add(category)
    await db.commit()
    result = await db.execute(select(Category).options(selectinload(Category.subcategories)).where(Category.id == category.id))
...
async def admin_update_category(category_id: int, body: CategoryUpdate, _: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    category = await db.scalar(select(Category).where(Category.id == category_id).with_for_update())
    if not category:
...
async def admin_create_subcategory(body: SubcategoryCreate, _: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    if not await db.scalar(select(Category.id).where(Category.id == body.category_id)):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Категория не найдена")
    exists = await db.scalar(select(Subcategory.id).where(Subcategory.category_id == body.category_id, Subcategory.slug == body.slug))
    if exists:
...
async def admin_update_subcategory(subcategory_id: int, body: SubcategoryUpdate, _: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    subcategory = await db.scalar(select(Subcategory).where(Subcategory.id == subcategory_id).with_for_update())
    if not subcategory:
    ...
    if not await db.scalar(select(Category.id).where(Category.id == next_category_id)):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Категория не найдена")
    exists = await db.scalar(select(Subcategory.id).where(Subcategory.category_id == next_category_id, Subcategory.slug == next_slug, Subcategory.id != subcategory_id))
    if exists:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Подкатегория с таким slug уже существует")
...
async def admin_list_reviews(skip: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=100), _: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Review).options(joinedload(Review.author), joinedload(Review.target_user)).order_by(Review.created_at.desc()).offset(skip).limit(limit))
