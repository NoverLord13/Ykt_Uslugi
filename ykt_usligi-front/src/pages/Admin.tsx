...
export const Admin = () => {
  ...
  const load = async () => {
    try {
      const [userData, serviceData, reportData, reviewData] = await Promise.all([api.adminGetUsers(), api.adminGetServices(), api.adminGetReports(), api.adminGetReviews()]);
      setUsers(userData); setServices(serviceData); setReports(reportData); setReviews(reviewData);
