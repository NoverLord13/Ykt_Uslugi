...
import { api, fileUrl, formatPrice, getApiErrorMessage, type ServiceSummary, type Review, type UserProfile } from '../api/Api';
...
export const Profile = () => {
  ...
  const [profile, setProfile] = useState<UserProfile | null>(null); const [ads, setAds] = useState<ServiceSummary[]>([]); const [reviews, setReviews] = useState<Review[]>([]);
  ...
  const [adsHasMore, setAdsHasMore] = useState(true); const [reviewsHasMore, setReviewsHasMore] = useState(true);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setAds(userAds); setReviews(userReviews); setAdsHasMore(userAds.length === 50); setReviewsHasMore(userReviews.length === 50);
      } catch (err) {
  ...
  const loadMoreProfileData = async () => { if (!profile) return; try { const [moreAds, moreReviews] = await Promise.all([adsHasMore ? api.getUserServices(profile.id, ads.length, 50) : Promise.resolve([]), reviewsHasMore ? api.getUserReviews(profile.id, reviews.length, 50) : Promise.resolve([])]); setAds(items => [...items, ...moreAds]); setReviews(items => [...items, ...moreReviews]); setAdsHasMore(moreAds.length === 50); setReviewsHasMore(moreReviews.length === 50); } catch (err) { setError(getApiErrorMessage(err, 'Не удалось загрузить продолжение профиля')); } };
  ...
  return <div className="page-shell">
    ...
    <section className="surface relative overflow-hidden p-5 sm:p-8"><div className="absolute -right-20 -top-24 h-64 w-64 rounded-full bg-[var(--brand-soft)] blur-2xl" />
      <div className="relative flex flex-col gap-6 sm:flex-row sm:items-start">
        <div className="grid h-28 w-28 flex-none place-items-center overflow-hidden rounded-[30px] bg-[var(--accent)] text-3xl font-black text-white shadow-xl shadow-blue-200/60">{profile.avatar_url ? <img loading="lazy" decoding="async" src={fileUrl(profile.avatar_thumbnail_url || profile.avatar_url)} alt={name} className="h-full w-full object-cover" /> : initials}</div>
        <div className="min-w-0 flex-1"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="eyebrow">{isOwn ? 'Ваш публичный профиль' : 'Профиль специалиста'}</p><h1 className="mt-2 text-3xl font-black tracking-tight text-[var(--ink)] sm:text-4xl">{name}</h1><p className="mt-1 text-sm text-[var(--muted)]">@{profile.username}{profile.location ? ` · ${profile.location}` : ''}</p></div><div className="flex gap-2">{isOwn ? <button onClick={() => setEditing(true)} className="button-primary">Редактировать</button> : getToken() && <button onClick={() => setReport({ type: 'user', id: profile.id })} className="button-secondary">Сообщить о проблеме</button>}</div></div>
          ...
          <div className="mt-6 flex flex-wrap gap-3"><div className="rounded-2xl bg-[#f4f6f7] px-4 py-3"><b className="block text-xl text-[var(--ink)]">{profile.performer_rating_avg?.toFixed(1) || '—'}</b><span className="text-xs text-[var(--muted)]">как исполнитель · {profile.performer_reviews_count || 0}</span></div><div className="rounded-2xl bg-[#f4f6f7] px-4 py-3"><b className="block text-xl text-[var(--ink)]">{profile.customer_rating_avg?.toFixed(1) || '—'}</b><span className="text-xs text-[var(--muted)]">как заказчик · {profile.customer_reviews_count || 0}</span></div><div className="rounded-2xl bg-[#f4f6f7] px-4 py-3"><b className="block text-xl text-[var(--ink)]">{ads.length}</b><span className="text-xs text-[var(--muted)]">активных объявлений</span></div></div>
        </div>
      </div>
    </section>
    ...
    <div className="mt-10 grid gap-10 lg:grid-cols-[1.1fr_.9fr]">
      <section><div className="mb-5 flex items-end justify-between"><div><p className="eyebrow">Витрина</p><h2 className="mt-1 text-2xl font-black">Объявления</h2></div>{isOwn && <Link to="/my-ads" className="text-sm font-bold text-[var(--brand)]">Управлять →</Link>}</div>{ads.length ? <div className="grid gap-4 sm:grid-cols-2">{ads.map(item => <Link key={item.id} to={`/services/${item.id}`} className="surface group overflow-hidden transition hover:-translate-y-1"><div className="aspect-[16/10] overflow-hidden bg-[#f1edf4]">{(item.image_url || item.images[0]?.url) ? <img loading="lazy" decoding="async" src={fileUrl(item.images[0]?.thumbnail_url || item.image_thumbnail_url || item.images[0]?.url || item.image_url)} alt={item.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="grid h-full place-items-center text-3xl">✦</div>}</div><div className="p-4"><h3 className="line-clamp-2 font-black">{item.title}</h3><p className="mt-2 font-black text-[var(--brand)]">{formatPrice(item)}</p></div></Link>)}</div> : <div className="empty-state">Активных объявлений пока нет.</div>}</section>
      <section><div className="mb-5"><p className="eyebrow">Репутация</p><h2 className="mt-1 text-2xl font-black">Отзывы</h2></div>{reviews.length ? <div className="space-y-4">{reviews.map(item => <article key={item.id} className="surface p-5"><div className="flex items-center justify-between gap-3"><div><p className="font-black">{item.author.display_name || item.author.username}</p><p className="text-xs text-[var(--muted)]">Подтверждённая сделка · {item.review_type === 'performer' ? 'об исполнителе' : 'о заказчике'}</p></div><span className="status-pill bg-[#fff3dc] text-[#9a5b00]">★ {item.rating}.0</span></div>{item.text && <p className="mt-4 text-sm leading-6 text-[#445064]">{item.text}</p>}{getToken() && item.author.id !== profile.id && <button onClick={() => setReport({ type: 'review', id: item.id })} className="mt-3 text-xs font-bold text-[var(--muted)] hover:text-[var(--danger)]">Пожаловаться</button>}</article>)}</div> : <div className="empty-state">После завершённых сделок здесь появятся отзывы.</div>}</section>
    </div>
    {(adsHasMore || reviewsHasMore) && <div className="mt-6 text-center"><button onClick={() => void loadMoreProfileData()} className="button-secondary">Показать ещё</button></div>}
